...
class Departments extends Controller
{
    ...
    public function category($name){
        $category = [];
        $category = explode('_', $name);
        $title = ucfirst($category[0]) . ' ' . ucfirst($category[1]);
        switch($name){
            case 'trending_gifts':
                $filters = DB::table('sub_categories')
     ...
     * @return \Illuminate\Http\Response
     */
    public function department_gifts(Request $request)
    {
        if($request->ajax()){
            if($request->action == 'department-gifts'){
                $output = $gift_count = '';
                $result = [];
                ...
                if(!empty($request->latest) && $request->latest == 'created_at'){
                    $result = DB::table('gifts')
                                ...
                                ->where('gifts.category_id', $request->category)
                ...
                if(!empty($filter)){
                    if($filter == 'under-25'){
                        $result = DB::table('gifts')
                                    ...
                                    ->select('gifts.*', 'gifts.id as gift_id', 'gifts.slug as gift_slug', 'categories.*', 'sub_categories.*', 'sub_categories.name as sub_category')
                                    ->where([
                                        ['gifts.category_id', '=', $request->category],
                                        ['usd_price', '<', 25]
                                    ])
                                    ->orderBy('usd_price', $request->price_ordering)
                                    ->distinct()
                ...
                && empty($request->likes) && empty($request->trending && $request->currency == 'usd')){
                    $result = DB::table('gifts')
                                ...
                                ->join('sub_categories', 'sub_categories.id', '=', 'gifts.sub_category_id')
                                ->select('gifts.*', 'gifts.id as gift_id', 'gifts.slug as gift_slug', 'categories.*', 'sub_categories.*', 'sub_categories.name as sub_category')
                                ->whereIn('gifts.category_id', $request->category)
                                ->orderBy('usd_price', $request->price_ordering)
                                ->distinct()
                                ->get();
                }
