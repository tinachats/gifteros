...
<script>
    $(function(){
        ...
        function subcategoryGifts(start, limit, sub_category_id, filter, min_price, max_price, rating, price_ordering, latest, likes, trending, currency) {
            var action = 'sub-category-gifts';
            $.ajax({
                url: '{{ route("subcategory_gifts") }}',
                method: 'post',
                data: {
                    action: action,
                ...
                success: function(data) {
                    if(data.result.length > 0){
                        status = 'inactive';
                        $('#sub-category-gifts').append(data.gifts);
        ...
        function filteredGifts(sub_category_id, filter, min_price, max_price, rating, price_ordering, latest, likes, trending, currency) {
            var action = 'sub-category-gifts';
            $.ajax({
                url: '{{ route("subcategory_gifts") }}',
                method: 'post',
                ...
                success: function(data) {
                    if(data.gifts){
                        ...
                        $('#fuzzy-loader').addClass('d-none');
                        $('.gifts-preloader').html(' ');
                        $('#sub-category-gifts').html(data.gifts);
                        $('#gift-count').html(data.gift_count);
                        userCurrency();
                    }
                }
        ...
        $(document).on('click', '.price-filter', function(e){
            ...
            filteredGifts(sub_category_id, filter, min_price, max_price, rating, price_ordering, latest, likes, trending, currency);
        ...
        if(status == 'inactive'){
            status = 'active';
            subcategoryGifts(start, limit, sub_category_id, filter, min_price, max_price, rating, price_ordering, latest, likes, trending, currency);
        }
