...
$(function() {
    // Product Preloader
    (function productsPreloader(limit) {
        var output = '';
    ...
    // Search preloader
    (function searchPreloader(limit = 7) {
        var output = '';
        for (var i = 0; i < limit; i++) {
            output += `
                ...
                <li class="list-group-item rounded-0 lh-100 px-1 py-2 cart-menu-item">
                    <div class="d-flex justify-content-between align-items-start">
                        ...
                        <div class="d-block text-center">
                            ...
                            <button class="content-placeholder btn btn-cart-placeholder pt-1"></button>
                        </div>
                    </div>
                </li>
                <!-- /.Cart Item Placeholder -->
        ...
        $('#searh-list').html(output);
    })();

    (function reviewsPreloader(limit = 4) {
        var output = '';
        for (var i = 0; i < 4; i++) {
            output += `
    ...
    // App Rating
    $('.toggle-ratingbox').click(function(e) {
        e.preventDefault();
    ...
    $(document).on('click', '.compare-btn', function() {
        ...
        var product_img = $('#image' + product_id).val();
        var usd_price = $('#usd-price' + product_id).val();
        var zar_price = $('#zar-price' + product_id).val();
        var zwl_price = $('#zwl-price' + product_id).val();
        var product_category = $('#category-name' + product_id).val();
        var product_quantity = $('#quantity' + product_id).val();
        var product_units = $('#product-units' + product_id).val();
        var product_description = $('#description' + product_id).val();
        var action = 'compare-item';

        if ($('#compare-btn' + product_id).hasClass('active')) {
            $('#compare-btn' + product_id).removeClass('active');
            $('#compare-btn' + product_id).html('<i class="material-icons text-primary mr-1">compare_arrows</i> Compare');
            var product = document.querySelector('#product-pane' + product_id);
            ...
            $('#compare-btn' + product_id).addClass('active');
            $('#compare-btn' + product_id).html('<i class="material-icons mr-1">done</i> Selected');
            $.ajax({
                url: 'actions.php',
                method: 'post',
                data: {
                ...
                success: function() {
                    ...
                    product.id = 'product-pane' + product_id;
                    product.innerHTML = `
                        <div class="media p-2">
                            <span role="button" class="material-icons text-danger remove-comparison" id="${product_id}" title="Remove product">cancel</span>
                            <img src="dist/img/products/${product_img}" height="80" width="80" alt="${product_name}">
    ...
    $(document).on('click', '.visitor-wishes', function() {
        var product_id = $(this).attr('id');
        var product_name = $(this).data('name');
        $('#wishlist-modal').modal('show');
        $('#visitor-wish-item').html(product_name);
        $('#wishlist-modal-signin').attr('href', 'index.php?redirect=details.php?id=' + product_id);
        $('#wishlist-modal-signup').attr('href', 'signup.php?redirect=details.php?id=' + product_id);
    });
    ...
    // Show review modal
    $('#show-write-review').click(function() {
    ...
    $(document).on('click', '.details-wishlist-btn', function() {
        ...
        wishlistGift(action, product_id, user_id);
    });
    ...
    $(document).on('click', '.helpful-btn', function() {
        ...
        var product_id = $(this).data('id');
        var action = $(this).data('action');
        postHelpful(action, product_id, rating_id);
    });
    ...
    $(document).on('click', '.unhelpful-btn', function() {
        ...
        var product_id = $(this).data('id');
        var action = $(this).data('action');
        postUnhelpful(action, product_id, rating_id);
    });
...
function searchItem(obj) {
    ...
    let search = obj.value;
    if (val > 0) {
        $('.line').css('display', 'block');
        $('#search-list').css('display', 'block');
        var action = 'search-product';
        $.ajax({
            url: '{{ route("GiftSearch.action") }}',
            method: 'post',
            data: {
                action: action,
                search: search
            ...
            dataType: 'json',
            success: function(data) {
                $('#search-list').html(data.search_results);
                userCurrency();
            }
        });
    } else {
