...
var printing_usd_cost = printing_zar_cost = printing_zwl_cost = 0;
var gift_id, post_id, user_id, gift_name, filter, customization_type;
var gift_quantity = 1;

// Current tab is set to be the first custom-tab (0)
var currentTab = 0;
...
$(function() {
    ...
    // Token handling and refreshments
    appTokens();
    $.ajaxSetup({
        headers: {
    ...
    $(document).on('click', '.compare-btn', function() {
        ...
        if ($('#compare-btn' + gift_id).hasClass('active')) {
            ...
            var gift = document.querySelector('#product-pane' + gift_id);
            ...
            gift_pane.replaceChild(placeholder_pane, gift);

            // remove the gift item from the comparison session
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.message == 'success') {
                        // Create gift comparison
                        let gift_pane = document.querySelector('.compare-products');
                        let gift = document.createElement('div');
                        gift.classList = 'product-pane filled box-shadow-sm';
                        gift.id = 'product-pane' + gift_id;
                        gift.innerHTML = `
                            <div class="media p-2">
                        ...
                        gift_pane.replaceChild(gift, placeholder_pane);
    ...
    $('.expire-session').on('click', function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.message == 'session-cleared') {
    ...
    $(document).on('click', '.add-to-cart-btn', function(e) {
        ...
        zar_price = parseFloat($('#zar-price' + gift_id).val());
        zwl_price = parseFloat($('#zwl-price' + gift_id).val());
        sale_usd_price = parseFloat($('#sale-usd-price' + gift_id).val());
        sale_zar_price = parseFloat($('#sale-zar-price' + gift_id).val());
        sale_zwl_price = parseFloat($('#sale-zwl-price' + gift_id).val());
...
// Add into cart animation
function addIntoBox() {
    $('#gift-added-modal').modal('show');
    setTimeout(function() {
        $('#gift-added-modal').modal('hide');
    }, 4000);
}
...
// Gift customization animation
function cardAnimation() {
    $('#animated-card-modal').modal('show');
    $('#add-greeting-card').addClass('d-none');
    productDetails(product_id);
    customizedGift(product_id);

}

// Added wrapper animation
function boxAnimation() {
    $('#wrapper-customizing-modal').modal('hide');
    $('.upload-progress-modal').modal('hide');
    $('#wrapper-form')[0].reset();
    $('#animated-box-modal').modal('show');
}

// Added Accessory Animation
function accessoryAnimation() {
    $('#gift-added-modal').modal('show');
    setTimeout(() => {
        $('#gift-added-modal').modal('hide');
    }, 1500);
...
function wrapperCompleteHandler(e) {
    if (e.target.responseText == 'success') {
        boxAnimation();
        customizedGift(product_id);
        $('#go-to-accessories').click(function(e) {
            $('#animated-box-modal').modal('hide');
            window.location.href = `http://localhost/targets/details.php?id=${product_id}#pills-accessories`;
        });
    } else {
        $('.upload-progress-modal').modal('hide');
        iziToast.warning({
...
shoppingCart(usd_delivery, zar_delivery, zwl_delivery, usd_coupon, zar_coupon, zwl_coupon);

function shoppingCart(usd_delivery, zar_delivery, zwl_delivery, usd_coupon, zar_coupon, zwl_coupon) {
    ...
    $.ajax({
        ...
        data: {
            action: action,
            usd_delivery: usd_delivery,
            zar_delivery: zar_delivery,
            zwl_delivery: zwl_delivery,
            usd_coupon: usd_coupon,
            zar_coupon: zar_coupon,
            zwl_coupon: zwl_coupon,
            _token: _token
        },
        dataType: 'json',
        success: function(data) {
            if (data.message == 'success') {
                ...
                $('#usd-total-cart').val(usd_total);
                $('#zar-total-cart').val(zar_total);
                $('#zwl-total-cart').val(zwl_total);
                $('.gift-count').val(count_cart);
                $('.cart-count').text(count_cart);
                $('.shopping-bag').html(data.shopping_cart);
                $('.additional-charges').html(data.additional_charges);
                userCurrency();
            }
        }
    });
...
function addToCart(gift_id, gift_name, gift_image, usd_price, zar_price, zwl_price,
    ...
    if (gift_quantity > 0) {
        $.ajax({
            ...
            success: function(data) {
                if (data.message === 'success') {
                    $('.gift-count').html(data.count_cart);
...
function subtractFromCart(gift_id, gift_quantity) {
    var action = 'decrease-product';
    if (gift_quantity > 0) {
        $.ajax({
            url: 'actions.php',
            method: 'post',
