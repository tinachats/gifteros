// Checkout cart details .popover
var gift_id, post_id, user_id, gift_name;
var filter, customization_type;
var gift_quantity = 1;
var gift_ids = [];
...
$(function() {
    ...
    // Toggle active class on scroll-item
    $('.scroll-item').on('click', function() {
        $('.scroll-item').not(this).removeClass('active');
        $(this).addClass('active');
    });

    // Add a fill class to icons
    ...
    // Toggle the active class when an icon is clicked
    $('.nav-menu-item').on('click', function() {
        $('.nav-menu-item').not(this).removeClass('active');
        $(this).addClass('active');
    });

    $('.sticky-nav-item').on('click', function() {
        $('.sticky-nav-item').not(this).removeClass('active');
        $(this).addClass('active');
    });
    ...
    $(document).on('click', function(e) {
        ...
        if (!app_menu.is(e.target) && app_menu.has(e.target).length == 0) {
    ...
    // Close the sidebar when user click somewhere else
    $(document).on('mouseup', function(e) {
    ...
    $(document).on('click', '.add-to-cart-btn', function(e) {
        ...
        zar_price = parseFloat($('#zar-price' + gift_id).val());
        zwl_price = parseFloat($('#zwl-price' + gift_id).val());
        var sale_percentage = $('#end-time' + gift_id).val();
        var gift_quantity = $('#quantity' + gift_id).val();
        var gift_units = $('#product-units' + gift_id).val();
        ...
        addToCart(gift_id, gift_name, gift_image, usd_price, zar_price, zwl_price,
            sale_percentage, gift_quantity, gift_units, category_name);
        iziToast.show({
            animateInside: true,
            theme: 'dark',
            timeout: 3000,
...
function countdownTimer(gift_id, countdown_date, now) {
    ...
    var x = setInterval(() => {
        ...
        if (diff < 0) {
            clearInterval(x);
            $('#countdown-timer' + gift_id).addClass('text-danger');
            $('#countdown-timer' + gift_id).html('Sale closed');
            $('#sale-icon' + gift_id).html('alarm_off');
            $('#sale-status' + gift_id).html('SALE CLOSED');

            // Update the sale percentage
            var action = 'sale-closed';
            $.ajax({
                url: '/close-sale',
                method: 'post',
                data: {
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.message == 'success') {
                ...
                $('#viewed-gifts').html(data.viewed_gifts);
...
function userCurrency() {
    ...
    if (currency) {
        if (currency == 'usd') {
            ...
            $('#currency').val('usd');
            $('#min-price').attr('placeholder', '$Min');
            $('#max-price').attr('placeholder', '$Max');
            ...
            $('.zwl-price').addClass('d-none');
            $('#currency').val('zar');
            $('#min-price').attr('placeholder', 'RMin');
            $('#max-price').attr('placeholder', 'RMax');
...
function mobilePhones() {
    if (window.innerWidth < 768) {
        // Functions and methods for mobile viewers
    }
}